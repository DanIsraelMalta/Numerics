...
namespace PointDistance {
    ...
    **/
    ...
    namespace TwoD {
        ...
        /**
        * \brief return the signed distance of a point from circle
        * @param {Vector2,        in}  point
        * @param {Vector2,        in}  circle center
        * @param {floating_point, in}  circle radius
        * @param {floating_point, out} signed distance
        **/
        template<typename T>
            requires(std::is_floating_point_v<T>)
        constexpr T circle_sdf(const GLSL::Vector2<T>& p, const GLSL::Vector2<T>& c, const T r) noexcept {
        ...
        **/
        template<typename T>
            requires(std::is_floating_point_v<T>)
        constexpr T rectangle_at_center_sdf(const GLSL::Vector2<T>& p, const GLSL::Vector2<T>& b) noexcept {
            assert(b.x > T{});
            assert(b.y > T{});
            constexpr GLSL::Vector2<T> zero(T{});
        ...
        **/
        ...
        constexpr T segment_sdf(const GLSL::Vector2<T>& p, const GLSL::Vector2<T>& a, const GLSL::Vector2<T>& b) {
            ...
            const T dot{ GLSL::dot(ba) };
            assert(!Numerics::areEquals(dot, T{}));
            const T h{ Numerics::min(Numerics::max(GLSL::dot(pa, ba) / dot, T{}), static_cast<T>(1)) };
        ...
        **/
        template<typename T>
            requires(std::is_floating_point_v<T>)
        constexpr T sdf_ellipse(const GLSL::Vector2<T> p, const GLSL::Vector2<T> ab) {
    ...
    namespace ThreeD {
        ...
        /**
        * \brief return the signed distance of sphere located
        * @param {Vector3,        in}  point
        * @param {Vector3,        in}  sphere center
        * @param {floating_point, in}  sphere radius
        * @param {floating_point, out} signed distance
        **/
        template<typename T>
            requires(std::is_floating_point_v<T>)
        constexpr T sdf_sphere(const GLSL::Vector3<T>& p, const GLSL::Vector3<T>& c, const T s) noexcept{
            return (GLSL::length(p - c) - s);
        }
        ...
        **/
        ...
        constexpr T sdf_to_cone(const GLSL::Vector3<T>& p, const GLSL::Vector2<T>& c, const T h) {
            ...
            const GLSL::Vector2<T> a(w - q * Numerics::clamp<T{}, one>(GLSL::dot(w, q) / GLSL::dot(q)));
            const GLSL::Vector2<T> b(w - q * GLSL::Vector2<T>(Numerics::clamp<T{}, one>(w.x / q.x), one));
        ...
        **/
        ...
        constexpr T sdf_to_capsule(const GLSL::Vector3<T>& p, const GLSL::Vector3<T>& a, const GLSL::Vector3<T>& b, const T r) {
            ...
            const T h{ Numerics::clamp<T{}, static_cast<T>(1)>(GLSL::dot(pa, ba) / GLSL::dot(ba))};
        ...
        **/
        ...
        constexpr auto udf_to_triangle(const GLSL::Vector3<T>& p, const GLSL::Vector3<T>& a, const GLSL::Vector3<T>& b, const GLSL::Vector3<T>& c) {
            ...
            return std::sqrt(
                ...
                ?
                Numerics::min(
                    GLSL::dot(ba * Numerics::clamp<T{}, one>(GLSL::dot(ba, pa) / GLSL::dot(ba)) - pa),
                    GLSL::dot(cb * Numerics::clamp<T{}, one>(GLSL::dot(cb, pb) / GLSL::dot(cb)) - pb),
                    GLSL::dot(ac * Numerics::clamp<T{}, one>(GLSL::dot(ac, pc) / GLSL::dot(ac)) - pc))
                :
        ...
        constexpr auto udf_to_quad(const GLSL::Vector3<T>& p, const GLSL::Vector3<T>& a, const GLSL::Vector3<T>& b, const GLSL::Vector3<T>& c, const GLSL::Vector3<T>& d) {
            ...
            return std::sqrt(
                ...
                Numerics::min(
                    GLSL::dot(ba * Numerics::clamp<T{}, one>(GLSL::dot(ba, pa) / GLSL::dot(ba)) - pa),
                    GLSL::dot(cb * Numerics::clamp<T{}, one>(GLSL::dot(cb, pb) / GLSL::dot(cb)) - pb),
                    GLSL::dot(dc * Numerics::clamp<T{}, one>(GLSL::dot(dc, pc) / GLSL::dot(dc)) - pc),
                    GLSL::dot(ad * Numerics::clamp<T{}, one>(GLSL::dot(ad, pd) / GLSL::dot(ad)) - pd))
                :
                GLSL::dot(nor, pa) * GLSL::dot(nor, pa) / GLSL::dot(nor));
        }
    }
